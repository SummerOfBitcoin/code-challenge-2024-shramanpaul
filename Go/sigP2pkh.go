...
func SerializeP2ph(filename string) {
	...
	for idx := range tx.Vin {
		...
		if tx.Vin[idx].Prevout.ScriptpubkeyType == "p2pkh" || tx.Vin[idx].Prevout.ScriptpubkeyType == "p2sh" {
			...
			// tx.Vin[idx].Scriptsig = hex.EncodeToString(scriptSig)
			// // Convert sigAppendsighash and scriptpubkey to byte slices do it for p2sh
			// sigBytes, err := hex.DecodeString(sigAppendsighash)
			// if err != nil {
			// 	fmt.Println("Error:", err)
			// 	return
			// }
			// scriptpubkeyBytes, err := hex.DecodeString(tx.Vin[idx].Prevout.Scriptpubkey)
			// if err != nil {
			// 	fmt.Println("Error:", err)
			// 	return
			// }

			// // Prepend the length of sigAppendsighash and scriptpubkey
			// sigBytes = append([]byte{byte(len(sigBytes))}, sigBytes...)
			// scriptpubkeyBytes = append([]byte{byte(len(scriptpubkeyBytes))}, scriptpubkeyBytes...)

			// // Append the byte slices together to form the witness
			// witness := append(sigBytes, scriptpubkeyBytes...)
